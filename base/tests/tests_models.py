...
def test_client_field_lengths():
    client = Client.objects.create(name="Имя" * 12, phone="1" * 15)  # Предполагая макс. длину name = 50, phone = 15
...
def test_booking_guests_and_rooms_choices():
    booking = Booking.objects.create(
        name="Тест", phone="123456789",
        date_in=date.today(), date_out=date.today() + timedelta(days=1),
        guests=2, rooms=3
    )
...
def test_news_ordering():
    ...
    older_news = News.objects.create(
        title="Старая новость", content="Содержание старой новости", author=user,
        pub_date=now() - timedelta(days=1)
    )
    newer_news = News.objects.create(
        title="Новая новость", content="Содержание новой новости", author=user,
        pub_date=now()
    )
