...
class BookingForm(forms.ModelForm):
    ...
    def clean_phone(self):
        ...
        if not (phone.startswith('+7') or phone.startswith('8')) or len(phone) != 12:
            raise ValidationError('Введите корректный номер телефона, начинающийся с +7 или 8 и содержащий 11 цифр.')
        return phone
    ...
    def save(self, commit=True):
        ...
        if missing_fields:
            error_message = _('Следующие поля обязательны к заполнению: ') + ', '.join(missing_fields)
